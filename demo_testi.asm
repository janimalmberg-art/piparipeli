...
PLAYER_X_HI     = $f8   ; UUSI: Spriten X-koordinaatin yläbitti
TEXT_PTR_LO     = $f9
TEXT_PTR_HI     = $fa
PLAYER_X        = $fb
PLAYER_Y        = $fc
...
InitGame:
    ...
    lda #160
    sta PLAYER_X
    lda #0          ; Nollataan ylabitti alussa
    sta PLAYER_X_HI
    lda #150
    sta PLAYER_Y
...
CheckKitchenZones:
    ...
    ; HUOM: Koska laajensimme aluetta yli 255, tarkistus pelkalla PLAYER_X
    ; saattaa olla epavarma jos olemme oikeassa laidassa (X>255 mutta lowbyte <100).
    ; Mutta tässä pelilogiikassa taikinakone on vasemmalla, eli X_HI=0 vaaditaan.
    lda PLAYER_X_HI
    bne .chk_table          ; Jos X>255, ollaan varmasti oikealla -> tarkista poyta
...
.chk_table:
    ; 2. LEIVONTAPOYTA (yla-oikea): X>220 tai X_HI=1, Y<100
    ; Tarkistetaan ensin ollaanko oikealla puolella ruutua
    lda PLAYER_X_HI
    bne .is_right_side      ; Jos HI=1, ollaan >255 eli oikealla
    lda PLAYER_X
    cmp #220
    bcs .is_right_side      ; Jos X>=220, ollaan oikealla
    
    jmp .chk_oven           ; Muuten vasemmalla/keskella -> tarkista uuni
...
.chk_oven:
    ; 3. UUNI (ala-oikea): X>220 tai HI=1, Y>180
    ; Tiedamme jo etta emme ole "varmasti poydalla", tarkistetaan X uudestaan
    lda PLAYER_X_HI
    bne .is_oven_x
    lda PLAYER_X
    cmp #220
    bcs .is_oven_x
    jmp .chk_register       ; Ei uunillakaan
...
.chk_register:
    ...
    lda PLAYER_X_HI
    bne .not_reg            ; Jos HI=1, ei voi olla kassa
    lda PLAYER_X
    cmp #100
    bcc .reg_x_ok
.not_reg:
...
.check_a:
    ...
    ; UUSI VASEN: lataa -1 ($FF) ja kutsu MovePlayerSpriteX
    lda #$ff
    jsr MovePlayerSpriteX
...
.check_d:
    ...
    ; UUSI OIKEA: lataa 1 ja kutsu MovePlayerSpriteX
    lda #1
    jsr MovePlayerSpriteX
...
MovePlayerSpriteX:
    ...
    adc PLAYER_X
    sta PLAYER_X
    bcc .moveXDone      ; Ei ylivuotoa -> valmis
    
    ; Ylivuoto (255 -> 0): Aseta MSB-bitti paalle
    lda PLAYER_X_HI
    ora #$01
    sta PLAYER_X_HI
    rts
...
.moveleft:
    ; Liiku vasemmalle (A < 0)
    clc                 ; ADC negatiivisella luvulla vaatii CLC jotta ylivuoto (carry) toimii oikein tarkistuksessa
    adc PLAYER_X
    sta PLAYER_X
    bcs .moveXDone      ; Carry set = tulos >= 0, ei alivuotoa
    
    ; Alivuoto (0 -> 255): Nollaa MSB-bitti
    lda PLAYER_X_HI
    and #$fe
    sta PLAYER_X_HI
    rts
